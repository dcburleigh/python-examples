...
class DB:
    ...
    #fields = ['project', 'application', 'date', 'user', 'product', 'product_version',  'details']
    cfg = None
    errors = []
    messages = []
    table_name = None
    ...
    def __init__(self,f=None, t=None):
        ...
        try:
            ...
            print("config file {} failed: {}".format( self.cfg_file, err))
        return
    ...
    def open(self):
        try:
            self.dbh = pymysql.connect(
                ...
                user=self.cfg.getByPath('user'),
                password=self.cfg.getByPath('password'),
